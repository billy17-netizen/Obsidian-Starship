...
- Compact keybind menu: set `KeybindMenuWidth`, `KeybindMenuHeight`, `KeybindMenuMaxHeight`, `KeybindMenuSize`, or call `Library:SetKeybindMenuSize()` to keep the draggable keybind menu from expanding across the screen.
- Layout modes: set `TabsMode = "Sidebar"` or `"Topbar"` and `TabStyle = "Card"` for card-like tab buttons.
- Sprite/video URL media: `Library:DownloadSprite(url)` and `Library:DownloadVideo(url)` download direct URLs through `getcustomasset`; `Groupbox:AddSprite()` and `Library:AddFloatingSprite()` animate sprite sheets with atlas frames/size/columns. Floating sprites render on a dedicated high-display-order layer so they are not clipped by windows/tabs.
- Drawing canvas elements: `Groupbox:AddCanvas()`, `AddGraph()`, `AddPlayerCard()`, and `AddTopUserBox()` add clipped custom UI surfaces for dashboard graphs, player detail cards, and ranked user boxes.
- Hidden tab cards: `Window:AddHiddenTab()` creates tabs that do not appear in the tab holder; pair it with `Tab:AddCard({ TargetTab = hiddenTab })` for card-index navigation.
